...
def add_cherry(number_of_cherrys):
    for i in range(number_of_cherrys):
        fill(255,0,0)
        min_x = width/2-SCOOPSIZE/2 + 10
        max_x = SCOOPSIZE/3 +width/2 +10
        min_y = coneY-((SCOOPSIZE)*scoops)/2-40
        max_y = coneY
        sprinkle_area_x = random(x = 30, y = 40)
        sprinkle_area_y = random(x = 20, y= 40)
        sprinkle_width = random(90,120)
        sprinkle_height = random(90,120)
        ellipse(sprinkle_area_x,sprinkle_area_y,sprinkle_height,sprinkle_width)
"""
